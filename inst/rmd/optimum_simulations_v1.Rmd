...
stan_logreg <- stan_model("stan/logreg.stan")
stan_logreg_np <- stan_model("stan/logreg_nopred.stan")
```
...
The end-point is food allergy at 18-months. For simplicity, assume babies are enrolled and randomised at 0 months of age. Then no follow-up data is available until 18-months after the first infant is enrolled.

Suppose accrual is 20 infants per week, then, by the time we have follow-up on the first individual we will have enrolled 1,560 infants (78 weeks $\times$ 20 per week). The minimum acrrual rate needed to enroll 3,000 infants over 5 years is about 11.5 per week. At this rate, by the time we have follow-up on the first individual we would have enrolled 780 infants.
...
d$interim <- as.numeric(d$followup %in% seq(500, 3000, 100))
d$anyfollowup <- as.numeric(d$followup > 0)

ggplot(d,
       aes(x = week)) +
  geom_line(aes(y = enrolled, colour = "Enrolled")) +
  geom_line(aes(y = followup, colour = "Responded"), linetype = 2) +
  ...
  labs(x = "Week", y = "Number enrolled/follow-up")
...
sce <- sim_scenario(500, p1tru = 0.11, p2tru = 0.1, n1int = n_foll, n2int = n_foll)

system.time(sce_ppos <- calc_scenario_ppos(sce, k_ppos = 0.95, 
                                        post_method = "approx", pp_sim = 1e4, ppos_name = "ppos_final"))
system.time(sce_ppos <- calc_scenario_ppos(sce_ppos, k_ppos = 0.95, 
                                            m1int = n_miss, m2int = n_miss, 
                                            post_method = "approx", pp_sim = 1e4, ppos_name = "ppos_interim"))
res <- decide_trial(sce_ppos, fut_k = seq(0.01, 0.1, length.out = 5))
