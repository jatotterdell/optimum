...
```{r, eval=FALSE, fig.cap="Assumed linear accrual rate and associated delay of information assuming 12-month end-point.", fig.height = 4}
H <- function(x) as.numeric(x>0)
...
The following tables present the expected sample size and estimates under each scenario
  ...
  * $\mathbb E(\text{enrolled})$ - the average number of enrolled at stopping time
  * $\text{Med}(\text{enrolled})$ - median number of enrolled at stopping time
  * $\mathbb E(\theta^a)$ - average value of estimate of $\theta^a$
  * $\mathbb E(\theta^w)$ - average value of estimate of $\theta^w$
