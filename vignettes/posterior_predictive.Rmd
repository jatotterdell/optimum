...
n_foll <- seq(250, 1500, 125)
n_enro <- pmin(1500, n_foll + 750)
...
sim_dat <- calc_scenario_ppos(sim_dat, ppos_name = "interim", m1 = n_enro, m2 = n_enro, post_method = "exact")
...
ptail_vb <- lapply(1:nrow(sim_dat), function(i) {
  ...
  ypred <- data.table(y1 = rbinom(1e3, sim_dat[i, final_m1], plogis(beta[, 1])),
                      y2 = rbinom(1e3, sim_dat[i, final_m2], plogis(beta[, 1] + beta[, 2])))
  ypred <- ypred[, .N, keyby = .(y1, y2)]
  for(j in 1:nrow(ypred)) {
    pred_fit <- vb_logistic_n(X, 
                              sim_dat[i, c(y1, y2)] + ypred[j, c(y1, y2)],
                              sim_dat[i, c(n1, n2)] + sim_dat[i, c(final_m1, final_m2)],
                              mu0 = c(0, 0), Sigma0 = diag(c(10, 10)), alg = "sj")
...
par(mfrow = c(2, 2))
hist(qlogis(rbeta(1e5, sim_dat[11, a1], sim_dat[11, b1])), freq = F, breaks = 100,
     main = "Beta-binomial posterior", xlab = "Log-odds")
curve(dnorm(x, m[1, 11], sqrt(s[1, 11])), add = TRUE)
legend("topleft", lty = 1, legend = "VB approx logistic model", bty = 'n')

hist(qlogis(rbeta(1e5, sim_dat[11, a2], sim_dat[11, b2])) -
     qlogis(rbeta(1e5, sim_dat[11, a1], sim_dat[11, b1])), 
     freq = F, breaks = 100, main = 'Beta-binomial posterior',
     xlab = "Difference in log-odds")
curve(dnorm(x, m[2, 11], sqrt(s[4, 11])), add = TRUE)
legend("topleft", lty = 1, legend = "VB approx logistic model", bty = 'n')

B <- rmvnorm(1e5, m[, 11], matrix(s[, 11], 2, 2))
hist(plogis(B[, 1]), freq = F, breaks = 100, 
     main = 'VB approx logistic model', xlab = bquote(theta[1]))
curve(dbeta(x, sim_dat[11, a1], sim_dat[11, b1]), add = TRUE)
legend("topright", lty = 1, legend = "Beta-binomial posterior", bty = 'n')

hist(plogis(B[, 1] + B[, 2]), freq = F, breaks = 100, 
     main = 'VB approx logistic model', xlab = bquote(theta[1]))
curve(dbeta(x, sim_dat[11, a2], sim_dat[11, b2]), add = TRUE)
legend("topright", lty = 1, legend = "Beta-binomial posterior", bty = 'n')
```


```{r}
plot(sim_dat$ptail, sapply(ptail_vb, "[[", 1), xlim = c(0, 1), ylim = c(0, 1)); abline(0,1)
plot(sim_dat$final, sapply(ptail_vb, "[[", 2), xlim = c(0, 1), ylim = c(0, 1)); abline(0,1)

cbind(sim_dat[, .(ptail, final)], cbind(sapply(ptail_vb, "[[", 1), sapply(ptail_vb, "[[", 2)))
```